    ...
    with open(input_file_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            if len(fields) == len(headers):  # Ensure correct number of fields
                ...
                if target_pmids and (int(row["PMID"]) in target_pmids):
                    yield row
...
def load_pubmed_raw_filter_animal_save_for_ner(
    pubmed_content_path,
    output_path,
    target_pmids=None,
    data_folders=None,
    ...
    if target_pmids:
        ...
        output_file_base = (
            f"{output_path}/full_pubmed_filtered_animal_{len(target_pmids)}"
        )
    else:
    ...
    for folder in data_folders:
        ...
        for df_chunk in load_folder_files(
            folder_path,
            headers,
            separator,
            target_pmids,
            ...
            df_metadata.to_csv(
                f"{output_file_base}_metadata.csv",
            ...
            df_missing_abstract.to_csv(
                f"{output_file_base}_missing_abstract.csv",
                mode="w" if first_write else "a",
                header=first_write,
                index=False,
            )
...
def load_and_clean_study_type_teller_animal(file_path, output_file):
    df = pd.read_csv(
        file_path,
        names=["PMID", "label_id", "label_name", "confidence"],
        header=None,
    )
...
def split_ner_data_to_chunks(saved_file_name_ner, output_dir_ner_chunks):
    print(
        f"Splitting animal studies for NER inference from: "
        f"{saved_file_name_ner}"
    )
    ...
    for i, chunk in enumerate(
        ...
        output_chunk_file = os.path.join(
            output_dir,
            f"pubmed_filtered_animal_for_NER_chunk_{i + 1}.csv",
        )
    ...
    print(
        f"All chunks saved successfully to {output_dir_ner_chunks}."
    )


def main():
    files_path = "/shares/animalwelfare.crs.uzh/Preclinical_Pipeline"

    # Clean Animal studies classification output

    ## original setup
    # file_path = (
    #     "./02_animal_study_classification/model_predictions/"
    #     "all_animal_studies_complete.txt"
    # )
    # output_file_animal_studies = (
    #     "./02_animal_study_classification/model_predictions/"
    #     "all_animal_studies_clean_complete.csv"
    # )

    ## update 2025 studies
    file_path = (
        f"{files_path}/02_animal_study_classification/model_predictions/"
        f"all_animal_studies_update_2025.txt"
    )

    output_file_animal_studies = (
        f"{files_path}/02_animal_study_classification/model_predictions/"
        f"all_animal_studies_clean_update_2025.csv"
    )

    load_and_clean_study_type_teller_animal(
        file_path,
        output_file_animal_studies,
    )
    ...
    ## original setup
    # pubmed_content_path = (
    #     "./01_pubmed_query_neuro/data/full_pubmed_raw"
    # )
    # filtered_output_path = (
    #     "./02_animal_study_classification/data/animal_studies"
    # )
    # saved_file_name_ner = load_pubmed_raw_filter_animal_save_for_ner(
    #     pubmed_content_path,
    #     filtered_output_path,
    #     animal_pmids,
    # )
    ...
    ## update 2025 studies
    pubmed_content_path = (
        f"{files_path}/01_pubmed_query_neuro/data/full_pubmed_raw/"
    )

    filtered_output_path = (
        f"{files_path}/02_animal_study_classification/"
        f"data/animal_studies/update_2025"
    )
    ...
    saved_file_name_ner = load_pubmed_raw_filter_animal_save_for_ner(
        ...
        filtered_output_path,
        animal_pmids,
        data_folders=["update_2025"],
    )

    output_dir_ner_chunks = (
        f"{files_path}/02_animal_study_classification/"
        f"data/animal_studies_for_ner/update_2025"
    )

    split_ner_data_to_chunks(
        saved_file_name_ner,
        output_dir_ner_chunks,
    )
