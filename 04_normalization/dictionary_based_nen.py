...
def process_conditions(df_pred_unique):
    term_file_path = "04_normalization/data/term_dictionaries/diseases_dictionary_mesh_icd_2024.csv"
    synonyms_dict = generate_conditions_lookup_dictionary(term_file_path)

    df_conditions_mapped, match_counts, processed_counts, same_condition_counts = process_dataframe(df_pred_unique, synonyms_dict, "unique_conditions_linkbert_predictions")
    ...
    counts_df_condition.to_csv("04_normalization/nen_stats/condition_dict_mapped.csv", index=False)

    return df_conditions_mapped

def process_interventions(df_conditions_mapped):
    ...
    df_drugs_mapped, match_counts, processed_counts, same_condition_counts = process_dataframe(df_conditions_mapped, drug_variant_to_canonical, "unique_interventions_linkbert_predictions", "drugs")
    ...
    counts_df_interventions.to_csv("04_normalization/nen_stats/intervention_dict_mapped.csv", index=False)

    return df_drugs_mapped


def main():
    ner_outputs_path = "03_ner/data/animal_studies_with_drug_disease/filtered_df_non_empty_595768.csv"
