...
def extract_abbreviation_from_full_text(pmid_set, folder_path = "data/animal_studies_for_ner_inference", save_to_path="03_ner/data/abbreviations_expansion/pmid_abbreviations.csv"):
...
def get_emtpy_ner_stats(df_pred):
    ...
    results_empty_entities.to_csv("03_ner/ner_stats/empty_ner_predictions_count.csv")
...
def main():
    ...
    save_abbrev_to_path = "03_ner/data/abbreviations_expansion/pmid_abbreviations.csv"
    if not os.path.isfile(save_abbrev_to_path):
        extract_abbreviation_from_full_text(pmid_set, "02_animal_study_classification/data/animal_studies_for_ner", save_abbrev_to_path)
    ...
    # save all articles
    save_dir = "03_ner/data"
    filtered_df_non_empty[["PMID", "unique_conditions_linkbert_predictions", "unique_interventions_linkbert_predictions"]].to_csv(f'{save_dir}/animal_studies_with_drug_disease/filtered_df_non_empty_{len(filtered_df_non_empty)}.csv', index=False)
    filtered_df_non_empty[["PMID"]].to_csv(
    f"{save_dir}/animal_studies_with_drug_disease/filtered_df_non_empty_{len(filtered_df_non_empty)}_PMIDs.csv",
    index=False
    )
